...
class GeminiNode:
    ...
    def generate_content(self, prompt, input_type="image", model_version="gemini-2.0-flash-exp", 
                      ...
                      aspect_ratio="none", random=False):
        ...
        generation_config = types.GenerationConfig(
            ...
            temperature=temperature,
            seed=seed  # Add seed to all requests for consistency
        )
        ...
        try:
            if chat_mode:
                ...
                if input_type == "text":
                    content = prompt
                elif input_type == "image" and images is not None:
                    # Prepare images using our improved function
                    pil_images = prepare_batch_images(images, max_images)
                    if len(pil_images) > 0:
                        # Use the first image as example for chat
                        content = [f"Analyze this image: {prompt}", pil_images[0]]
                    else:
                        content = prompt
                else:
                    # Use the prepare_response utility for other types
                ...
                if input_type == "image" and images is not None and isinstance(images, torch.Tensor) and images.nelement() > 0:
                    # Prepare images directly when using image input
                    pil_images = prepare_batch_images(images, max_images)
                    
                    if len(pil_images) > 0:
                        # Create parts for multimodal input
                        if len(pil_images) > 1:
                            content_text = f"Analyze these {len(pil_images)} images. {prompt}"
                        else:
                            content_text = prompt
                    ...
                    # Use the prepare_response utility for other types
                    content_parts = prepare_response(prompt, input_type, None, images, video, audio, max_images)
                
                # Add structured output instruction if requested
                if structured_output and isinstance(content_parts, list) and len(content_parts) > 0:
                    if isinstance(content_parts[0], dict) and "parts" in content_parts[0] and len(content_parts[0]["parts"]) > 0:
                        if isinstance(content_parts[0]["parts"][0], dict) and "text" in content_parts[0]["parts"][0]:
                            content_parts[0]["parts"][0]["text"] = f"Please provide the response in a structured format. {content_parts[0]['parts'][0]['text']}"
    ...
    def generate_images(self, prompt, model_version, images=None, batch_count=1, 
                        temperature=0.4, seed=0, max_images=6, aspect_ratio="none", random=False):
            ...
            for i in range(batch_count):
                try:
                    ...
                    if batch_images:
                        ...
                        status_text += f"Batch {i+1} (seed {current_seed}): Generated {len(batch_images)} images\n"
                    else:
